...
### 4.0 起動モード選択
- 起動時にモード選択画面を表示する。
- モード選択画面では「おうちレジ」「カメラ」の2つのアイコンボタンを表示する。
- 「おうちレジ」を選ぶと通常画面へ遷移し、従来の会計機能を利用できる。
- 「カメラ」を選ぶとカメラモードへ遷移する。
...
### 4.5 音の再生
- モード選択時、商品登録時、明細消去時、決済時にそれぞれ効果音を再生する。
- 音量と効果音パターンは調整可能とする。
...
### 4.8 カメラモード
- カメラモード中は常時プレビュー表示を行う。
- カメラモード中は操作ガイド文を常時表示しない。
- カメラモード中に画面タップするとシャッター効果音を再生し、静止画表示へ切り替える。
- 静止画表示中は写真らしい外枠を表示する。
- 静止画表示中に再度タップするとプレビュー表示へ戻る。
...
## 6. 状態要件
- アプリは起動モード選択状態、おうちレジ状態、カメラモード状態を持つ。
- おうちレジ状態は通常状態と決済完了表示状態を持つ。
- おうちレジの通常状態では商品登録、決済、明細消去を受け付ける。
- おうちレジの決済完了表示状態では入力受付を停止する。
- おうちレジの決済完了表示状態は一定時間後に自動終了する。
- カメラモード状態はプレビュー表示状態と静止画表示状態を持つ。
...
## 8. 受け入れ条件
- 起動直後にモード選択画面が表示されること。
- 起動直後に「おうちレジ」を選ぶと通常画面が表示され、明細0件・合計0円であること。
- 起動直後に「カメラ」を選ぶとカメラプレビューが表示されること。
- カメラプレビュー表示中に画面タップで静止画表示へ切り替わること。
- 静止画表示中に画面タップでカメラプレビューへ戻ること。
